module.exports = {
    ...
    async execute({ chatJid, db }) {
        ...
        try {
            ...
            const modoAtual = db.config.obterConfiguracaoGrupo(chatJid, 'modo_so_adm') === 'true';

            // Alterna o estado (toggle)
            if (modoAtual) {
                // Se está ativo, desativa
                db.config.salvarConfiguracaoGrupo(chatJid, 'modo_so_adm', 'false');
                ...
                db.config.salvarConfiguracaoGrupo(chatJid, 'modo_so_adm', 'true');
